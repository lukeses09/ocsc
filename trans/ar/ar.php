...
<?php include("aside.php") ?>
      ...
      <div class="content-wrapper">
        ...
        <div class="row" style="margin-top:30px; display:none">
          ...
          <div id="dateInputDiv" class="col-md-4">
                <input type="text" class="form-control input-lg" id="dateInput" placeholder="Select Range Date" />          
          </div><!--/. col 4-->
          <div class="col-md-1">
            <button id="btnSearchDate" class="btn btn-lg btn-default" style="background-color:white"><i class="fa fa-search"></i> Search </button>
          </div>   

          <div class="col-md-3"></div>    
        </div> <!-- /. row -->

        <div class="row" id="lblDiv" style="margin-top:20px; display:block">  <!-- bar labels below-->

          <div class="col-md-3 col-xs-12"> <!-- total customer -->
            <div class="info-box">
              <span class="info-box-icon bg-aqua"><i class="ion-ios-pie-outline"></i></span>
              <div class="info-box-content">
                <span class="info-box-text">Total Records</span>
                <h1 id="lbl_tot_rec" class="info-box-number"></h1>
              </div><!-- /.info-box-content -->
            </div><!-- /.info-box -->
          </div><!--/.col(0)-->

          <div class="col-md-3 col-xs-12"> <!-- total sales -->
            <div class="info-box">
              <span class="info-box-icon bg-yellow"><i class="ion-podium"></i></span>
              <div class="info-box-content">
                <span class="info-box-text">Total Balance</span>
                <h1 id="lbl_tot_bal" class="info-box-number"></h1>
              </div><!-- /.info-box-content -->
            </div><!-- /.info-box -->      
          </div><!--/.col(2)-->
          ...
          <div class="col-md-3 col-xs-12"> <!-- total sales -->
            <div class="info-box">
              ...
              <div class="info-box-content">
                ...
                <h1 id="lbl_tot_current" class="info-box-number"></h1>
              </div><!-- /.info-box-content -->
            </div><!-- /.info-box -->      
          </div><!--/.col(2)-->          
          ...
          <div class="col-md-3 col-xs-12"> <!-- total sales -->
            <div class="info-box">
              <span class="info-box-icon bg-red"><i class="ion-ios-paper-outline"></i></span>
              <div class="info-box-content">
          ...
          <div class="row" id="tablediv" style="display:block">                     <!-- TABLES -->
                  ...
                  <table id="jsontable" class="table table-condensed table-striped table-hover">
                    <thead>
                      <tr>
                        ...
                        <th style="width:130px">ACC No.</th>      
                        <th> Customer </th>              
                        <th>Balance</th>
                        <th>Current</th>
                        <th>Receivable</th>
                    ...
                    <tfoot>
                      ...
                      <th id="foot_bal"></th>
                      <th id="foot_current"></th>
                      <th id="foot_receivable"></th>
                    </tfoot>
                  </table>
                </div><!-- /.box-body -->
              </div><!-- /.box -->
...
function load(getStart,getEnd){  // LOAD DATATABLES
    $('#loading').modal({backdrop: 'static'})              
    $('#loading').modal('show');
    var s = getStart;
    var e = getEnd;
    var dataString = 'startDate='+s+'&endDate='+e;    
      ...
      console.log(s);
          ...
          var bal=0;
            ...
            {          
              ...
              current = parseFloat(current,10) + parseFloat(s[i][4],10);
              bal = parseFloat(bal,10) + parseFloat(s[i][3],10);              
              oTable.fnAddData
              ([
                s[i][0],s[i][1],s[i][2],comma(s[i][3]),comma(s[i][4]),comma(s[i][5]),
              ],false);        
              oTable.fnDraw();                 
            } // End For    
          
          $('#lbl_tot_rec').html(comma(oTable.fnGetData().length));
          $('#lbl_tot_amount').html(comma(tot.toFixed(2)));
          $('#lbl_tot_current').html(comma(current.toFixed(2)));
          $('#lbl_tot_bal').html(comma(bal.toFixed(2)));     
          $('#foot_receivable').html(comma(tot.toFixed(2)));
          $('#foot_current').html(comma(current.toFixed(2)));
          $('#foot_bal').html(comma(bal.toFixed(2)));                         
      },
      error: function(e){
        $('#loading').modal('hide');                                             
...
function viewRemittance(getStart,getEnd)
{
  load(getStart,getEnd);
...
$('#btnSearchDate').click(function(){   //searchButton .Clicked
  ...
  if((str.length == 23 && str!='') ) // if meron both
  {    
    var startDate = str.slice(0,10);
    var endDate = str.slice(13);  
  $('#dateInputDiv').removeClass('has-error');    
  $('#jsontable').dataTable().fnClearTable();    
  viewRemittance(startDate,endDate);  
  }

  else if(str.length != 23){  // if walang date&match
  $('#dateInputDiv').addClass('has-error');
  $('#jsontable').dataTable().fnClearTable();    
  //lblReset();    
  }
  else if(str.length != 23 ){  // if walang date
  $('#dateInputDiv').addClass('has-error');
  $('#jsontable').dataTable().fnClearTable();    
  //lblReset();    
  }
  else if((str.length == 23 && str!='') ){  // if walang match
