...
$sql = "UPDATE charge SET charge_status = ? WHERE charge_id = ?";
$q = $conn -> prepare($sql);
...
$q = $conn -> prepare($sql);    
$q -> execute(array($trail_id,'Maintenance','Charges', 'REMOVE', 'Deleted ID: '.$id, date('Y/m/d H:i:s'),  $_SESSION["ocsc_uname"]  ));
